...
function copyForRole(role: MembershipRole): RoleCopy {
  if (role === 'surveyor') {
    return {
      preheader:
        'Welcome to BLD — your surveyor account is ready. Finish your portfolio to get matched.',
      heroSub:
        "You're in. Complete your portfolio so clients can find you — anywhere, on your schedule.",
      ctaLabel: 'Finish your portfolio',
      ctaPath: '/onboarding',
      intro:
        'Thanks for joining <strong style="color:#2A2558;">BLD</strong> &mdash; the managed marketplace connecting clients with independent site surveyors. Here&rsquo;s how it works.',
      feature1Title: 'Build your portfolio',
      feature1Body: 'Services, coverage, and rates &mdash; so you show up in the right matches.',
      feature2Title: 'Get matched',
      feature2Body: 'Receive project offers that fit your skills and service area.',
      feature3Title: 'Track every request',
      feature3Body: 'Review offers, accept work, and follow projects from one dashboard.',
      stripTitle: 'Ready when you are',
      stripBody: 'Your surveyor workspace is set up and waiting.',
      stripCta: 'Open portfolio',
      textBlurb:
        'Complete a few quick steps, then build your portfolio so the right projects can find you.',
    };
  }

  return {
    preheader:
      'Welcome to BLD — your account is ready. Post your first survey in minutes.',
    heroSub:
    ...
    feature1Title: 'Post your project',
    feature1Body: 'Scope, location, timeline &mdash; in under 5 minutes.',
    feature2Title: 'Get matched',
    feature2Body: 'We connect you to vetted surveyors near your site.',
    feature3Title: 'Track it end to end',
    feature3Body:
      'Follow progress, review deliverables, and manage everything from one dashboard.',
    stripTitle: 'Ready when you are',
    stripBody: 'Your dashboard is set up and waiting.',
    stripCta: 'Open dashboard',
    textBlurb:
...
/**
 * Branded welcome message after Create account (BLD marketing template).
 * Placeholders are filled server-side; motion CSS is progressive enhancement.
 */
export function buildWelcomeEmail(input: {
  ...
  const copy = copyForRole(input.role);
  const dashboardUrl = `${base}${copy.ctaPath}`;
  ...
  const text = [
    `Welcome to BLD, ${name}`,
    '',
    copy.heroSub.replace(/&rsquo;/g, "'").replace(/&mdash;/g, '—'),
    '',
    copy.textBlurb,
    '',
  ...
  const html = WELCOME_HTML_TEMPLATE.replaceAll('{{first_name}}', escapeHtml(name))
    .replaceAll('{{preheader}}', escapeHtml(copy.preheader))
    .replaceAll('{{hero_sub}}', escapeHtml(copy.heroSub))
    .replaceAll('{{cta_label}}', escapeHtml(copy.ctaLabel))
    .replaceAll('{{dashboard_url}}', escapeHtml(dashboardUrl))
    .replaceAll('{{intro_html}}', copy.intro)
    .replaceAll('{{feature1_title}}', escapeHtml(copy.feature1Title))
    .replaceAll('{{feature1_body}}', copy.feature1Body)
    .replaceAll('{{feature2_title}}', escapeHtml(copy.feature2Title))
    .replaceAll('{{feature2_body}}', copy.feature2Body)
    .replaceAll('{{feature3_title}}', escapeHtml(copy.feature3Title))
    .replaceAll('{{feature3_body}}', escapeHtml(copy.feature3Body))
    .replaceAll('{{strip_title}}', escapeHtml(copy.stripTitle))
    .replaceAll('{{strip_body}}', escapeHtml(copy.stripBody))
    .replaceAll('{{strip_cta}}', escapeHtml(copy.stripCta))
    .replaceAll('{{unsubscribe}}', escapeHtml(unsubscribeUrl))
    .replaceAll('{{privacy_url}}', escapeHtml(privacyUrl))
    .replaceAll('{{current_year}}', year);

  return { subject, text, html };
}

/** Exact BLD welcome layout (client/surveyor copy injected via placeholders). */
                      ...
                      <tr>
                        <td align="center" class="cta-wrap btn-solid" style="border-radius:12px; background-color:#FFFFFF; box-shadow:0 8px 20px rgba(42,37,88,0.24);">
                          ...
                          <a href="{{dashboard_url}}" target="_blank" style="display:inline-block; position:relative; padding:16px 40px; font-family:'DM Sans',Arial,sans-serif; font-size:15px; font-weight:700; color:#5B52E0; border-radius:12px; letter-spacing:0.01em;">
                            {{cta_label}} &nbsp;<span class="cta-arrow">&rarr;</span>
                          </a>
                        </td>
                      </tr>
                    </table>
                          ...
                          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="card" style="background:linear-gradient(180deg,#FBFBFF 0%,#EEEAFF 100%); border:1px solid rgba(91,82,224,0.12); border-radius:16px;">
                            <tr>
                              <td style="padding:22px 20px; font-family:'DM Sans',Arial,sans-serif;">
                                ...
                                <p style="margin:0 0 4px 0; font-size:15px; font-weight:700; color:#2A2558;">{{feature1_title}}</p>
                                <p style="margin:0; font-size:13.5px; line-height:20px; color:#6B668C;">{{feature1_body}}</p>
                              </td>
                            </tr>
                          </table>
                        </td>
                        <td class="col col-last" valign="top" width="50%" style="padding-left:8px;">
                          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="card" style="background:linear-gradient(180deg,#FBFBFF 0%,#EEEAFF 100%); border:1px solid rgba(91,82,224,0.12); border-radius:16px;">
                            <tr>
                              <td style="padding:22px 20px; font-family:'DM Sans',Arial,sans-serif;">
                                <div class="icon-tile" style="width:44px; height:44px; background-color:#FFFFFF; border-radius:12px; text-align:center; line-height:44px; font-size:22px; margin-bottom:14px; box-shadow:0 4px 10px rgba(42,37,88,0.10);">&#129309;</div>
                                <p style="margin:0 0 4px 0; font-size:15px; font-weight:700; color:#2A2558;">{{feature2_title}}</p>
                                <p style="margin:0; font-size:13.5px; line-height:20px; color:#6B668C;">{{feature2_body}}</p>
                              </td>
                            </tr>
                          </table>
                        </td>
                          ...
                          <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                            <tr>
                              ...
                              <td valign="top" style="padding-left:14px;">
                                <p style="margin:0 0 4px 0; font-size:15px; font-weight:700; color:#2A2558;">{{feature3_title}}</p>
                                <p style="margin:0; font-size:13.5px; line-height:20px; color:#6B668C;">{{feature3_body}}</p>
                              </td>
                            </tr>
                          </table>
                        </td>
                          ...
                          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                            <tr>
                              <td class="col" valign="middle" style="padding-right:12px;">
                                <p style="margin:0 0 4px 0; font-family:'Fraunces',Georgia,serif; font-size:18px; font-weight:700; color:#FFFFFF;">{{strip_title}}</p>
                                <p style="margin:0; font-size:13.5px; line-height:20px; color:#EEEAFF;">{{strip_body}}</p>
                              </td>
                                ...
                                <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="right">
                                  <tr>
                                    <td class="btn-ghost" style="border-radius:12px; background-color:#FFFFFF;">
                                      <a href="{{dashboard_url}}" target="_blank" style="display:inline-block; padding:13px 26px; font-family:'DM Sans',Arial,sans-serif; font-size:14px; font-weight:700; color:#5B52E0; border-radius:12px;">{{strip_cta}}</a>
                                    </td>
                                  </tr>
                                </table>
                              </td>
